...
def predict_rating(neighbors):
    ...
    """
    # TODO: implement average-based regression
    total = sum(neighbor["value"] for neighbor in neighbors)
...
def normalize(dataset):
    ...
    mins = [
        min(item["features"][i] for item in dataset)
        for i in range(num_features)
    ]
    maxs = [
        max(item["features"][i] for item in dataset)
        for i in range(num_features)
    ]
    ...
    for item in dataset:
        ...
        for i in range(num_features):
            ...
            max_val = maxs[i]
            if max_val == min_val:
                rescaled_features.append(0.0)
            else:
                rescaled_features.append(
                    (item["features"][i] - min_val) / (max_val - min_val)
                )

        new_item = dict(item)
        new_item["features"] = rescaled_features
        normalized_dataset.append(new_item)
...
def recommend(user, users, k):
    ...
    """
    # TODO: implement the recommend() pipeline
...
def main():
    ...
    for k in [1, 3, 15]:
        ...
        # each give a misleading answer here.
    ...
    # Explain in a comment why the raw and normalized predictions differ.
...
#    features would you extract from each restaurant?
# 2. What would go wrong if one of those features had the exact same
#    value for every restaurant in your dataset?
# ---------------------------------------------------------------------------
