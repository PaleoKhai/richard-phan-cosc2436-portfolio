...
def recursive_sum(arr):
    ...
    """
    # TODO: implement the base case check (e.g., if arr is empty, return 0)
    # TODO: implement the recursive case (first element + recursive_sum of the rest)
    if not arr:
        return 0
    return arr[0] + recursive_sum(arr[1:])  # placeholder so the function is compile-safe
...
def recursive_count(arr):
    ...
    Recursive case: TODO -- state how the problem shrinks each call
    """
    # TODO: implement the base case check (empty list -> 0)
    # TODO: implement the recursive case (1 + recursive_count of the rest)
    if not arr:
        return 0
    return 1 + recursive_count(arr[1:])  # placeholder so the function is compile-safe
...
def recursive_max(arr):
    ...
    Recursive case: TODO -- state how the problem shrinks each call
    """
    # TODO: if arr is empty, raise ValueError -- the maximum is undefined
    # TODO: implement the base case check (single element list -> that element)
    # TODO: implement the recursive case (compare first element to max of rest)
    if not arr:
    ...
    return arr[0] if arr[0] > sub_max else sub_max  # placeholder so the function is compile-safe
...
def binary_search_recursive(arr, target):
    ...
    """
    # TODO: write an inner helper(low, high) that does the recursion
    # TODO: implement base case(s) -- if low > high the range is empty, return -1
    # TODO: find the middle index with (low + high) // 2
    # TODO: compare arr[mid] to target and recurse on the correct half
    #       (recurse with mid + 1 or mid - 1, never mid, or the range won't shrink)
    def helper(low, high):
        if low > high:
            return -1
        mid = (low + high) // 2
    ...
    return helper(0, len(arr) - 1)  # placeholder so the function is compile-safe
...
def quicksort(array, pivot_strategy="first"):
    ...
    """
    # TODO: implement the base case (arrays of length < 2 are already sorted)

    # TODO: choose the pivot INDEX based on pivot_strategy
    #   "first"  -> 0
    #   "random" -> random.randrange(len(array))
    #   "middle" -> len(array) // 2
    #   anything else -> raise ValueError

    # TODO: save the pivot value, then build 'rest' -- every element EXCEPT the
    #       one at the pivot index. Remove by index, not by value: removing by
    #       value drops the wrong element when the list contains duplicates.

    # TODO: build 'less' (elements < pivot) and 'greater_or_equal' (>= pivot)
    #       from 'rest'. Using < and >= keeps every duplicate exactly once.

    # TODO: recursively sort both partitions, then concatenate
...
def measure_time(arr, pivot_strategy):
    ...
    """
    # TODO: record a start time using time.perf_counter()

    # TODO: call quicksort(list(arr), pivot_strategy) inside a try block so the
    #       original arr is untouched. Catch RecursionError and return None.

    # TODO: record the elapsed time (perf_counter() - start)

    # TODO: verify the result against Python's built-in sorted(arr).
    #       If they differ, raise RuntimeError -- a wrong answer must not be
    #       reported as a successful timing.
...
def run_benchmark(unsorted_list, sorted_list, reverse_sorted_list):
    ...
    # TODO: print a header row for the table (shape, strategy, result)
    print(f"{'shape':<18}{'strategy':<12}{'result':<18}")
    print("-" * 48)

    # TODO: loop over each (shape_name, data) pair in input_shapes.items()
    for shape_name, data in input_shapes.items():
    # TODO:   loop over each strategy in pivot_strategies
        for strategy in pivot_strategies:
    # TODO:     call measure_time(data, strategy)
            elapsed = measure_time(data,strategy)
    # TODO:     if the result is None, the result text is "RecursionError"
            if elapsed is None:
    ...
    # TODO:     otherwise format the elapsed seconds, e.g. f"{elapsed:.6f} s"
    # TODO:     print the shape name, strategy, and result text
