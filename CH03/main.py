...
def find_file(structure, target_name, current_path="", depth=0):
    ...
    # BASE CASE #1: a file node - either it's the one we want, or it isn't
    ...
    # RECURSIVE CASE: a folder - search each item inside it
    if structure["type"] == "folder":
        for item in structure["contents"]:
            ...
            if result is not None:
                trace_exit(new_path, depth)
                return result
        # BASE CASE #2: looped through everything in this folder, not found
        trace_exit(new_path, depth)
        return None
...
def count_files(structure):
    ...
    # RECURSIVE CASE: a folder contributes the sum of its contents
    if structure["type"] == "folder":
        total = 0
        for item in structure["contents"]:
...
def total_size(structure):
    ...
    # RECURSIVE CASE: a folder contributes the sum of its contents' sizes
    if structure["type"] == "folder":
        running_total = 0
        for item in structure["contents"]:
            running_total += total_size(item)
...
def print_tree_with_depth(structure, depth=0):
    ...
    # RECURSIVE CASE: print each item inside this folder, one level deeper
    if structure["type"] == "folder":
        for item in structure["contents"]:
