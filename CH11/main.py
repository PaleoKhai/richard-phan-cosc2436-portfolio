...
def dp_knapsack(items, capacity):
    ...
    for i in range(1, rows):
        ...
        for j in range(cols):
            best_without = grid[i - 1][j]

            best_with = 0
            if weight <= j:
                best_with = value + grid[i - 1][j - weight]

            grid[i][j] = max(best_without, best_with)

    best_value = grid[rows-1][cols-1]
    return grid, best_value

    # TODO: build the grid as a list of lists, rows x cols, initialized to 0
    #grid = [[0 for _ in range(cols)] for _ in range(rows)]

    # TODO: fill the grid row by row using the knapsack recurrence:
    #       grid[i][j] = best value using items up to row i with
    #       capacity j (either skip item i-1, or take it if it fits)
...
def longest_common_substring(a, b):
    ...
    start_index_in_a = end_index_in_a - max_length
    return a[start_index_in_a:end_index_in_a]
...
if __name__ == "__main__":
    ...
    print("TODO: describe the growth pattern here")
    ...
    print("TODO: explain why substring and subsequence can disagree")
